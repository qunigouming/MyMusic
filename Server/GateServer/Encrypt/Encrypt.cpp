...
bool Encrypt::VerifyHashes(const std::string& storedHashBase64, const std::string& receivedHashBase64)
{
	std::string storedHashDecoded, receivedHashDecoded;
	StringSource ss1(storedHashBase64, true, new Base64Decoder(new StringSink(storedHashDecoded)));
	StringSource ss2(receivedHashBase64, true, new Base64Decoder(new StringSink(receivedHashDecoded)));
	return VerifyBufsEqual(
		reinterpret_cast<const byte*>(storedHashDecoded.data()),
		reinterpret_cast<const byte*>(receivedHashDecoded.data()),
		storedHashDecoded.size()
	);
}
